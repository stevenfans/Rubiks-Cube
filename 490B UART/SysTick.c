...
void SysTick_Wait10ms(unsigned long delay){
  unsigned long i;
  for(i=0; i<delay; i++){
    SysTick_Wait(50000);  // wait 10ms (assumes 16 MHz clock)
  }
}
...
void SysTick_Wait500ms(unsigned long delay){
  unsigned long i;
  for(i=0; i<delay; i++){
    SysTick_Wait(5000000);  // wait 500ms (assumes 16 MHz clock)
  }
}

void SysTick_Wait2point5ms(unsigned long delay){
  unsigned long i;
  for(i=0; i<delay; i++){
    SysTick_Wait(40000);  // wait 10ms (assumes 16 MHz clock)
  }
}
