	...
	for (face = 0; face < 3; face++) {
		for (piece = 0; piece < 9; piece++) {
			for (int pix = 0; pix < 9; pix++) {      // (x,y,0,C)
				...
				// create vector of 9 red values, green values, blue values 
				test1.push_back(faces[face][piece][pix][R]);
				test2.push_back(faces[face][piece][pix][G]);
				test3.push_back(faces[face][piece][pix][B]);


				// get the averages of each vector
				if (test1.size() >= 9)
				{
					r = average(test1);
					g = average(test2);
